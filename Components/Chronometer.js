...
export default class Chronometer extends React.Component {
  ...
  _welcomeSound = async () => {
    ...
    try {
      audioObjectActions.setOnPlaybackStatusUpdate(
        this._onPlaybackStatusUpdateWelcome,
      );
  ...
  _initNeedle() {
    ...
    const positionTrigger =
      Math.round(Math.random() * (this._rNInit2 - this._rNInit1)) +
      this._rNInit1;
  ...
  _moveNeedle() {
    ...
    if (this._rN < this._rTimeLimit) {
      ...
      this.setState({
        displayTimeLimit:
          Math.round(
            (this.state.displayTimeLimit + TICK_INTERVAL / 1000) * 50,
          ) / 50,
      });
    } else {
  ...
  async _playRandomName() {
    if (this.state.isPlaying && this.state.gameStarted) {
      ...
      if (random < 90) {
        ...
        try {
          audioObjectNames.setOnPlaybackStatusUpdate(
            this._onPlaybackStatusUpdateName,
          );
  ...
  _updateRecordSips = (index, sipsDrank, sipsGiven, gpAction) => {
    if (!gpAction) {
      this.namesName[index].sipsDrank =
        this.namesName[index].sipsDrank + sipsDrank;
      this.namesName[index].sipsGiven =
        this.namesName[index].sipsGiven + sipsGiven;
    } else {
  ...
  async _playRandomAction() {
    if (this.state.isPlaying && this.state.gameStarted) {
      ...
      const audio = random.randomSipFolder(
        mod,
        this.props.records.actions.length,
        language,
        this.groupAction,
      );
      ...
      if (audio != 'UserAction') {
        try {
          ...
          audioObjectActions.setOnPlaybackStatusUpdate(
            this._onPlaybackStatusUpdateAction,
          );
        ...
        const index = Math.round(
          Math.random() * (this.props.records.actions.length - 1),
        );
    ...
    if (!playbackStatus.isLoaded) {
      ...
      if (playbackStatus.didJustFinish && !playbackStatus.isLooping) {
        ...
        if (this.originAudioRecorded == 'name') {
          if (this.state.isPlaying) {
            try {
              await audioObjectNames
                .pauseAsync()
                .then(() => this._playRandomAction());
            } catch (error) {
    ...
    if (!playbackStatus.isLoaded) {
      ...
      if (playbackStatus.didJustFinish && !playbackStatus.isLooping) {
        try {
          ...
          JSON.stringify(
            'Error pause/unload audioActio : ' + JSON.stringify(error, null, 2),
          );
        }
        // To fix duck on android
        this._initNeedle();
